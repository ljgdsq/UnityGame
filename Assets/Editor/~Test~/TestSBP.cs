...
public class TestSBP 
{
    ...
    [MenuItem("Test/UploadAllToCacheServer")]
    public static void UploadAllToCacheServer()
    {   
        CacheServerUploader.UploadAllFilesToCacheServer("127.0.0.1",8086);
    }
    
    [MenuItem("Test/StartACacheServer")]
    public static void StartACacheServer()
    {
        LocalCacheServer.Setup(1024*1024*1024,@"D:\CacheServerTest");
    }
        ...
        internal class LocalCacheServer : ScriptableSingleton<LocalCacheServer>
    {
        ...
        {
            var nodeExecutable = Path.Combine(EditorApplication.applicationContentsPath, "Tools", "nodejs");
            nodeExecutable = Application.platform == RuntimePlatform.WindowsEditor
                ? Path.Combine(nodeExecutable, "node.exe")
                : Path.Combine(nodeExecutable, "bin", "node");
            ...
            var cacheServerJs = Path.Combine(EditorApplication.applicationContentsPath, "Tools", "CacheServer", "main.js");
...
//                CreateNoWindow = true
        ...
        {
            Kill();
            instance.Create(port, size, cachePath);
