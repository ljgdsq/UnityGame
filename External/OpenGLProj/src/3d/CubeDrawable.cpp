...
CubeDrawable::CubeDrawable(const Shader &shader) : Drawable(shader) {
    ...
    VAO->addBufferLayout({0,3,GL_FLOAT, false,8* sizeof(float ),0});
    VAO->enableAttribute(0);

    VAO->addBufferLayout({1,3,GL_FLOAT, false,8* sizeof(float ),3* sizeof(float )});
    VAO->enableAttribute(1);

    VAO->addBufferLayout({2,2,GL_FLOAT, false,8* sizeof(float ),6* sizeof(float )});
